...
public struct Secp256k1 {
    ...
    public static func decompressPublicKey(_ pubkey: PublicKey) throws -> PublicKey {
        ...
        guard secp256k1_ec_pubkey_parse(context, &cPubkey, pubkey, pubkey.count) == 1,
              secp256k1_ec_pubkey_serialize(context,
                                            ...
                                            &uncompressedKeyLen,
                                            &cPubkey,
                                            Compression.uncompressed.flag) == 1 else {
            throw Error.internalError
        }
    ...
    public static func signCompact(msg: [UInt8], with privkey: PrivateKey, nonceFunction: NonceFunction) throws -> (sig: Signature, recID: RecoveryID) {
        ...
        guard secp256k1_ecdsa_sign_recoverable(context,
                                               &cSignature,
                                               msg,
                                               privkey,
                                               nonceFunction.function,
                                               nil) == 1 else {
            throw Error.internalError
        }
        ...
        guard secp256k1_ecdsa_recoverable_signature_serialize_compact(context,
                                                                      &signature,
                                                                      &recoveryID,
                                                                      &cSignature) == 1 else {
            throw Error.internalError
